...
from openpilot.system.bluetooth.bluez import BlueZClient
from openpilot.system.bluetooth.protocol import BLUETOOTH_SOCKET_PATH
from openpilot.system.bluetooth.radio import BluetoothRadio


OFFROAD_COMMANDS = {"set_power", "start_scan", "stop_scan", "pair", "forget", "test_audio", "pairing_response", "connect", "disconnect", "select_audio"}
SCAN_DURATION = 20.0
...
class BluetoothController:
  def __init__(self, params: Params | None = None, bluez_factory=BlueZClient, radio: BluetoothRadio | None = None,
    ...
    self.params_memory = params_memory or self.params
    ...
    self.params_memory.remove("BluetoothTestSound")
  ...
  def _client(self) -> BlueZClient:
    with self._lock:
      if self._bluez is None:
        ...
        self._bluez.agent.set_auto_accept_incoming(False)
        try:
          self._bluez.set_discoverable(False)
  ...
  def _test_audio_worker(self, address: str, deadline: float) -> None:
    try:
      ...
      if (not self._offroad() or not self.params.get_bool("BluetoothEnabled") or
          (self.params.get("BluetoothAudioAddress") or "").upper() != address.upper()):
        return
      device = self._client().device_for_address(address)
      if not device.get("connected"):
        return
      self.params_memory.put("BluetoothTestSound", "engage")
