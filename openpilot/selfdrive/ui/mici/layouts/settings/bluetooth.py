...
from openpilot.system.bluetooth.controller import ControllerMonitor
...
class BluetoothLayoutMici(NavScroller):
  def __init__(self):
    ...
    self._bluetooth_icon = gui_app.texture("icons_mici/settings.png", 56, 56)
    self._dialog_icon = gui_app.texture("icons_mici/settings.png", 64, 64)
  ...
  def _rebuild(self):
    ...
    self._power_btn.set_value(("on" if status.enabled else "off") if status.available else "Bluetooth OS support required")
  ...
  def _device_actions(self, device):
    ...
    if device.controller and self._manager.status.offroad:
      options.append("test controller")
    panel = NavScroller()
    def apply(action):
      ...
      elif action == "test controller":
        gui_app.push_widget(ControllerTestDialog())
    for option in options:
      button = BigButton(option, "", scroll=True)
      button.set_enabled(self._manager.status.offroad)
      button.set_click_callback(lambda action=option: apply(action))
      panel._scroller.add_widget(button)
...
class ControllerTestDialog(BigDialog):
  def __init__(self):
    super().__init__("controller test", "Button assignments are not configured")
    self._monitor = ControllerMonitor()
  ...
  def _render(self, rect):
    self._card.set_value(self._monitor.status)
    super()._render(rect)
